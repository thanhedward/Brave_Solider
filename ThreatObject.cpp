...
void ThreatsObject :: DoPlayer(Map& gMap)
{
    ...
    {
        come_back_time_--;
        if (come_back_time_ == 0 && type_move_ == MOVE_IN_SPACE_THREAT) // Quái thứ 2 bị rơi xuống vực nên khi init sẽ chuyển động như quái chạy do có hàm InitThreat
        {
            InitThreats();
        }
    }
    ...
    else
    {
        if (on_ground_ == true)
        {
            if (x_pos_ > animation_b_)
            {
                ...
                LoadImg("../assets/image/threat_left.png", screen);
            }
            else if (x_pos_ < animation_a_)
            {
                input_type_.left_ = 0;
                input_type_.right_ = 1;
                LoadImg("../assets/image/threat_right.png", screen);
            }
        }
        else
        {
